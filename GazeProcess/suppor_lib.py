...
def plot_scanpaths(scanpaths, img_path, lengths, save_path, img_height=256, img_witdth=512):
    ...
    for user_i in range(20):
        ...
        points_x = []
        points_y = []
        for sample_i in range(lengths[user_i]): #scanpath [y,x]
...
def plot_scanpaths_1(scanpaths, img_path, lengths, save_path, img_height=720, img_witdth=1440):
    # Plot predicted scanpaths
    # this code is on the basis of ScanGAN https://github.com/DaniMS-ZGZ/ScanGAN360/
    ...
    for user_i in range(20):
        ...
        for num, x, y, c in zip(range(0, len(points_x)), points_x, points_y, colors):
            ...
            if previous_point is not None:
                if abs(previous_point[0] - x) < (img_witdth / 2):
                    ax.plot([x, previous_point[0]], [y, previous_point[1]], color='blue', linewidth=4.,
                                         alpha=0.35)
                else:
                    h_diff = (y - previous_point[1]) / 2
                    if (x > previous_point[0]):  # X is on the right, Previous is on the Left
                        ax.plot([previous_point[0], 0],
                                             [previous_point[1], previous_point[1] + h_diff], color='blue',
                                             linewidth=4., alpha=0.35)
                        ax.plot([img_witdth, x], [previous_point[1] + h_diff, y],
                                             color='blue', linewidth=4., alpha=0.35)
                    else:
                        ax.plot([previous_point[0], img_witdth],
                                             [previous_point[1], previous_point[1] + h_diff], color='blue',
                                             linewidth=4., alpha=0.35)
                        ax.plot([0, x], [previous_point[1] + h_diff, y], color='blue', linewidth=4.,
                                             alpha=0.35)
            previous_point = [x, y]
        markersize = 8.
        for x, y, c in zip(points_x, points_y, colors):
            x = x * img_witdth
            y = y * img_height
            ax.plot(x, y, marker='o', markersize=markersize, color=c, alpha=.9)
        # ax.imshow(image)
        ax.axis('off')

        plt.savefig(save_path + '/sp_' + image_name + str(user_i) + ".png", bbox_inches='tight', pad_inches=0, dpi=500)
        plt.axis('off')
        plt.close('all')