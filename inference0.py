    ...
    def summary(self, samples):
        ...
        for index in range(int(len(samples) / 2)):
            ...
            if obs is not None:
                ...
                scanpath = torch.cat((scanpath, torch.unsqueeze(temp, dim=0)), dim=0)
            else:
                obs = torch.unsqueeze(xyz2plane(temp), dim=0)
                scanpath = torch.unsqueeze(temp, dim=0)

        # let ``n_scanpaths'' to be the first dim
        obs = torch.transpose(obs, 0, 1)  # [15,200,2]-->[200,15,2]  #y,x
        scanpath = torch.transpose(scanpath, 0, 1) #x,y,z
        ...
        for _, _, files in os.walk(self.img_path):
            ...
            for img in files:
                ...
                with torch.no_grad():
                    ...
                    # scanpaths,sp = self.summary(samples).cpu().numpy()
                    scanpaths,sp = self.summary(samples)
                    ...
                    print(sp.shape)
                    np.save(os.path.join(self.output_path, save_name), sp)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--model', default='./AOIv2_2+/model_lr-3e-05_bs-64_epoch-400.pkl', type=str,
    #                     help='model path, default = ./model/model_lr-0.0003_bs-64_epoch-435.pkl')
    parser.add_argument('--model', default='./AOI600_1-/model_lr-3e-05_bs-64_epoch-260.pkl', type=str,
                        help='model path, default = ./model/model_lr-0.0003_bs-64_epoch-435.pkl')
    parser.add_argument('--inDir', default='D:\csx\ScanPath\AOI\img_600_test', type=str,
                        help='image path, default = ')
    parser.add_argument('--outDir', default='./demo/AOI600/test_260_1+', type=str,
                        help='output path, default = ')
    parser.add_argument('--n_scanpaths', default=10, type=int,
